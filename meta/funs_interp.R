...
get_idw<-function(idp, data, base_shape,base_shape_int,coords, newgrid,newy,crs.info,...){
  ...
  interp_model=df_idw = idw(prev~1, data_spat, newgrid,idp=idp)
...
map_style<-function(data,get,coords, p,main="",subtitle="",cex.axes=13,cex.lab=13,cex.main=13,cex.sub=13,cex.leg=13,factors=NULL,cex.fac=13,col.fac="red",showcoords=T, col.coords="black",cex.coords=13,showguides=T, layer_shape=NULL,col.palette, leg="",layer_col="gray",lighten=1.4,newcolhabs,extralayers=NULL){
  ...
  if(isTRUE(is.factor(data[get][,1]))){
    ...
    p<-p+ geom_raster(data = rasterpoints , aes(x = x, y = y, fill = factor(z,levels=my_rst@data@attributes[[1]][,1], labels=my_rst@data@attributes[[1]][,2]))) +   scale_fill_manual(values=c(colhabs), name=leg, drop=F)
  } else{
    p=p+geom_raster(data = rasterpoints , aes(x = x, y = y, fill = z))+
  ...
  if(!is.null(layer_shape)){
    ...
    p<-p+geom_sf(data=layer, fill=lighten(layer_col,lighten), lty=1) + coord_sf(xlim = c(bbox[1,]), ylim = c(bbox[2,]), expand = FALSE)
  }
