  ...
  output$remove_sp<-renderUI({
    fluidRow(style="background: white; margin-top:10px",
             column(12,span("Value-based remotion",tipify(icon("fas fa-question-circle"), "Check to select a filter")),
                    column(12,id="value_rem",
                           div(inline(uiOutput("pct_rareabund"))),
                           div(inline(uiOutput("pct_rarefreq"))),
  ...
  output$pct_rareabund<-renderUI({
    fluidRow(
      ...
      inline( conditionalPanel("input.rareabund==T",{
        numericInput('pct_rare', NULL, value =0.1, width='70px')
      })),"%",
  ...
  output$pct_rarefreq<-renderUI({
    fluidRow(
      ...
      inline(conditionalPanel("input.rarefreq==T",{
        numericInput('pct_prev', NULL, value =0.1, width='70px')
      })),"%",
...
## transformations
 ...
 # onevent("mouseleave","upload_tools",shinyjs::removeClass('data_change',selector="body",class="data_change1"))
  ...
  output$histo<-renderUI({
    ...
    fixedPanel(id="histo",top = 0,right="100%",      uiOutput("track_change")
  ...
  observeEvent(input$tools_drop9,{
   ...
   # req(input$data_upload)
    ...
    div( id="upload_tools",
      ...
      span(id="tools_drop4",inline(dropdown(label=img(src=na_icon,size="sm",height='15',width='15'), status ="button_change" ,  inline = T, circle=F,width="350px",inputId="na_pop",uiOutput("pop_na")))),
      span(id="tools_drop5",inline(dropdown(label=img(src=split_icon,size="sm",height='20',width='20'), status ="button_change",inline=T, circle=F,width="300px",inputId='dropID_split',
                                           uiOutput("pop_split")
      ))),
