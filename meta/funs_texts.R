...
pwRDA2<-function (x.ord, y.ord, BPs, n.rand = 99)
{
  ...
  R.boot <- NULL
  pw.Models <- segRDA:::pwRDA.source(x.ord, y.ord, BPs)
  ...
  withProgress(message = "Running...",
               ...
               max = n.rand,
               {
                 ...
                 for (b in 1:n.rand) {
                   ...
                   R.boot[b] <- segRDA:::pwRDA.source(new.x, comm.rand, BPs)$summ[2]
...
tiphelp<-function(text){
  tipify(a(icon("fas fa-question-circle")),text)
}
pophelp<-function(title,text, placement="right"){
  popify(a(icon("fas fa-question-circle")),title,text, placement = placement, trigger="hover")
}
             ...
             column(12,
                    splitLayout(cellWidths = c('30%','70%'),
                                column(12,style="margin-top: 25px",
                                       div(strong(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'removed observations. If > 0 displays in the form of:: Original number of observations :: number of retained observations.',options=list(container="body"))
                                                  ,"Subobs"), style="margin: 7px"),
                                       div(strong(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'removed variables. If > 0 displays in the form of:: Original number of variables :: number of retained variables',options=list(container="body")),"Subvars"), style="margin: 7px"),
                                       div(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"), 'Transformation option from the Transformation toolbox',options=list(container="body")),strong("Transf"), style="margin: 7px"),
                                       div(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'Variables removed by frequency/abundance',options=list(container="body")),strong("Removed"), style="margin: 7px"),
                                       div(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'Scale option from the Transformation toolbox',options=list(container="body")),strong("Scale"), style="margin: 7px"),
                                       div(
                                         tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'Center option from the Transformation toolbox',options=list(container="body")),strong("Center"), style="margin: 7px"),
                                       div(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'NA.omit  option from the Transformation toolbox',options=list(container="body")),strong("NA.omit"), style="margin: 7px"),
                                       div(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'Data-Attribute Imputation',options=list(container="body")),strong("Data_imp"), style="margin: 7px"),
                                       div(tipify(placement = "left",icon("fas fa-question-circle",style="color: gray"),  'Factor-Attribute Imputation',options=list(container="body")),strong("Factor_imp"), style="margin: 7px")
                                ),
...
textoffline<-function(...)({
  column(12,
        h4(strong("Running offline"), icon("fab fa-windows"), icon("fab fa-apple"), icon("fab fa-linux")),
       ...
       column(12,style="background: white",
        ...
        p(style="border-top: 1px solid",
          h4(style="margin-top:20px",
           strong("Shell commands: Mac OS and Linux systems"), icon("fab fa-apple"), icon("fab fa-linux"))),
...
{
  column(12,
         p(code('singleton'),"Remove variables occurring only once.",icon("fas fa-exclamation-circle"), "Requires a counting data"),
...
## plot RDA
prda<-function(model,pch=c(16,3),key=NULL,keytext=NULL,points=T,text=F,palette="black",col.sp="red",col.arrow='steelblue', show.sp=T, scaling=1, constr=FALSE, cex.bp=1,col.lab="red",cex.text=.8,cex.points=1,textcolor="gray", biplot=T, n.sp=NULL, pch.sp=3, sp.display="Label",newcolhabs, pos=2, offset=0)
{
  ...
  plotrda<-plot(model,pch=pch[1],xlab=xlab,ylab=ylab,col=col, las=1, scaling=scaling)

  if(isTRUE(points)){points(model, display="sites", cex=cex.points, col=col, pch=pch[1], scaling=scaling)}
  if(isTRUE(text)){
    colkey2<-getcolhabs(newcolhabs,textcolor, nlevels(keytext))
    col2<-colkey2[keytext]
    text(model, display="sites",labels=keytext, cex=cex.text,  col=col2, scaling=scaling, pos=pos, offset=offset)}
  ...
  if(isTRUE(show.sp)){
    if(sp.display=="Label"){
      text(sp[which.sp,], col=col.sp, scaling=scaling, label=which.sp)
    } else{
      points(sp[which.sp,], pch=pch.sp,col=col.sp, scaling=scaling)
  ...
  if(isTRUE(biplot)){
    if(length(bp)>0){
      points(model, display="bp", axis.bp =T, scaling=scaling)
      text(model,display="bp", axis.bp =T, scaling=scaling)
    }}
