...
output$shp_control<-renderUI({
  ...
  output$remove_sp<-renderUI({
    column( 12,style="background: white",
            checkboxInput("mach_value",strong("Value-based remotion",tipify(icon("fas fa-question-circle"), "Check to select a filter"), style="color: black")),
            conditionalPanel("input.mach_value % 2",{
              splitLayout(cellWidths = c("60%","40%"),
                          column(12,radioButtons(
                            "rares", NULL,
                            choiceValues  = c("None","Abund<","Freq<","Singletons"),
                            choiceNames  = list(
                              list("None"),
                              list(tipify(span("Abund<"), "Remove variables with an value less than x-percent of the total")),
                              list(tipify(span("Freq<"),  "Remove variables occurring in less than  x-percent of the number of samples")),
                              list(tipify(span("Singletons"), "Requires a counting data. Remove variables occurring only once."))
                            )
                          )
                          ),
                          column(12,uiOutput("pct_remove"))
              )
            })
  ...
  output$pct_remove<-renderUI({
    if(input$rares=='Abund<'){
      numericInput('pct_rare', "%", value =0.1)

    } else if(input$rares=='Freq<'){numericInput('pct_prev', "%", value =0.1)} else if(input$rares=='Singletons'){
      if(sum(apply(data.rares(),2, is.integer), na.rm=T)==0){
  ...
  output$histo_plot<-renderUI({
    ...
    column(12,
      renderPlot({
        ...
        plothists(nadata_r$df,vals$newcolhabs, len=6)
...
if(length(input$selecvar) >0){
  ...
  data.rares<-reactive({
    ...
    if (length(input$rares) > 0) {
      ...
      if (input$rares == "Abund<") {data = pctRares(data, input$pct_rare/100)}
      if (input$rares == "Freq<") {data = pctPrev(data, input$pct_prev/100)}
    }
  ...
  output$tool1<-renderUI({
    ...
    fluidRow(id="tool1",
      ...
      column(12,
             uiOutput("teste_filter"),
            inline(
              div(
                div("Filter by factor level:"),
                div(id='filter_obs',style="text-align: top;vertical-align: top;border: 1px solid black",
                    tags$div(class = "inline",
                      lapply(get_faclevels(factors),function(x){
                        inline(
                          div(
                            div(

                              div(
                              checkboxInput(paste0("cut_fac",x[[2]]),x[[2]],F)),
                            div(
                              uiOutput(paste0("cutlev0_",x[[2]]))
                            ))
             ...
             inline(conditionalPanel("input.filter_data != 'none'",{inline(uiOutput("cutlevel_obs"))}))

             ),


      bsTooltip('filter_obs',"The factor to be used as filter"),

      inline(
        conditionalPanel("input.filterby=='Datalist'",{pickerInput("filter_datalist", NULL,choices = c("none",names(vals$saved_data)), width="200px")})
      ),

      column(12,
             column(12,uiOutput("filterby_indobs")),

             column(12,strong("Removed observations:"),style='white-space: normal;',
                    p(em("*observations where all cases are empty are removed by default",style='white-space: normal;')),
                    renderPrint({
                      IDs1=rownames(vals$saved_data[[input$data_upload]])
                      IDs2=rownames(getdata_upload())
                      IDs1[which(!IDs1%in%IDs2)]}))
  ...
  get_filterobs<-reactive({
    ids<-rownames( vals$saved_data[[input$data_upload]])

    factors<-attr(vals$saved_data[[input$data_upload]],"factors")
    l1<-get_faclevels(factors)
    res1<-lapply(l1,function(x){
      isTRUE(input[[paste0("cut_fac",x[[2]])]])
    })
    res2<-lapply(l1,function(x){
      input[[paste0('cutlev1_',x[[2]])]]
    })
    ...
    l2<-mapply(list, l1, res1, res2,as.list(factors),list(rownames(factors)),SIMPLIFY=FALSE)
    l3<-do.call(cbind, lapply(l2,function(x) {
      res<-x[[4]]%in%x[[3]]
      if(sum(res)==0){
        res<-T
      }
...
try({
  ...
  l3<-data.frame(l3)
  rownames(l3)<-rownames(factors)
  l3[l3==F]<-NA
  ids<-rownames(na.omit(l3))
},silent =T)
    ids
  ...
  observeEvent(input$data_upload,{
    ...
    lapply(l1,function(x) {
      output[[paste0("cutlev0_",x[[2]])]]<-
        renderUI({
          req(isTRUE(input[[paste0("cut_fac",x[[2]])]]))
          inline(checkboxGroupInput(paste0('cutlev1_',x[[2]]),NULL,x[[1]],selected=x[[1]]))
        })
  ...
  output$filterby_indobs<-renderUI({
    ...
    column(12,  style = 'overflow-x: scroll;height:250px;overflow-y: scroll',checkboxInput("check_obs",'Select/Unselect all',T),hr(),checkboxGroupInput("selecobs",NULL,choices = rownames(data),selected = rownames(data)))
  })
  ...
  observeEvent(input$undo_na,{
    nadata_r$df<-vals$saved_data[[input$data_upload]]
  })
  observeEvent(input$data_upload,{
    if(length(input$selecvar)>0)
  ...
  savechanges_comb<-reactive({
   # saveRDS(reactiveValuesToList(vals),"vals.rds")
   # saveRDS(reactiveValuesToList(input),"input.rds")
    #saveRDS(reactiveValuesToList(nadata_r),"nadata_r.rds")

   # vals<-readRDS("input.rds")
    #input<-readRDS("vals.rds")
   # nadata_r<-readRDS("nadata_r.rds")
