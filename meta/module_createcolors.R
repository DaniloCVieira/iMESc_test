...
module_server_cc <- function (input, output, session,vals,df_colors,newcolhabs){
  ns <- session$ns
  created_pal<-reactiveValues(df=NULL)
  newpalette<-reactiveValues(df_colors=NULL)



  output$newpalette_created<-renderUI({
    req(!is.null(created_pal$df))
    div(
  ...
  output$custom_palette<-renderUI({
    ...
    div(
      ...
      tipify( actionButton(ns("delete_palette"),icon("far fa-trash-alt")),"Click to delete the selected palette")
            ...
            div(
              div(class="cogs_in",
                  ...
                  inline(div(
                    ...
                    tipify(actionButton(ns("add_color"),icon("fas fa-level-down")),"Click to include the color")))
              ),
...
output$create_palette<-renderUI({
...
output$add_palette_button<-renderUI({
  req(length(newpalette$df_colors)>0)
  tipify(bsButton(ns("add_palette"),span(icon("fas fa-palette"),icon("fas fa-arrow-down"))),"Click to create a palette with the colors above. The new palette will be available in all color inputs")
})
       ...
       div(style=" background: #D1F2EB ; padding: 5px;",
           ...
           div(inline(div(style="width: 140px",
                          renderPlot({
                            ...
                            image(t(matrix(seq(0,1,length.out=100), nrow=1)), axes=F,col=  colorRampPalette(  newpalette$df_colors)(100))
                          }, height =30)))
           )
      ...
      div(style="width: 135px",
        inline(
          div(
            ...
            tipify(actionButton(ns("restart_palette"),span(icon("fas fa-undo"))),"Click to restart")
  ...
  output$newpalette_out<-renderPrint({
    newpalette$df_colors
  })
  maxcol<-reactive({
    max<-length(newpalette$df_colors)
    if(max<10){max=10}
    max
  })
  observeEvent(input$add_color,{
    req(maxcol()<=48)
    newpalette$df_colors<-c( newpalette$df_colors,input$col)
  })
  observeEvent(input$add_palette,{
    validate(need(!is.null(newpalette$df_colors),"Please use the arrow button to include the selected color in the new palette"))
    ...
    vals$newcolhabs[[palette_name]]<-colorRampPalette(newpalette$df_colors)
    created_pal$df<-T
    updatePickerInput(session,'available_palette',selected = palette_name)
    newpalette$df_colors<-NULL
  })
