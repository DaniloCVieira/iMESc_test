...
cutsom<-function(m,groups, members=NULL, method.hc="ward.D2", palette="matlab.like2",newcolhabs)
{
...
cutdata2<-function(data, groups, method.hc="ward.D2", dist="bray", palette="matlab.like2",newcolhabs)
{
  #validate(need(anyNA(data)==F, "The selected data contains missing values. Please remove them in Upload>Transform."))
  d<-vegdist(data, method=dist)
  hc<-hclust(d,method=method.hc)
