...
stack_scatter3D<-function(data=NULL,coords,base_shape, layer_shape,z=NULL,col_base="white", col_layer="gray",pt_col="gray",showlabels=F,showpoints=T,pt_cex=c(1,1,1,1),fac_cex=1,fac_col="red",symbol=16,scale_points=F,xlim=NULL, ylim=NULL,labels=NULL,  theta = 0, phi = 40, r = sqrt(3), d = 1,expand = 1, colkey=F,col.grid="gray", zlab="Value", scale_size=T, spacing=2, col.palette, newcolhabs,breaks=5, legwidth=50,cex.axis=1,cex.lab=1, zmin=1,zmax=length(data)+1,   ticktype="detailed",xlab="Longitude", ylab="Latitude", leglab.adj=.85,legtit.posy=1.15,legtit.posx=1,title.srt=0,lab.srt=0,col.labels="black",cex.labels=1,col.coords="black",cex.coords=1,show_coords=rep(T,length(data)), show_labels=rep(F,length(data))) {
  ...
  par(mar=c(1,1,1,1), xpd=T)
  {
    ...
    #col.palette<-list("matlab.like2","viridis","Grays","Blues")
    palettes<-list()
...

  {
    ...
    for(i in 1:length(get))
    {
      ...
      text(resmain, labels=names(data)[i], xpd=T,srt=title.srt)
    }
  }
}
...
stack4D_2<-function (rlist,zvalues=T,base_shape=NULL,layer_shape=NULL,space=1,transp=1,colgrid="gray",axes=c(1,2,3,4),show_coords=rep(F,length(rlist)), show_labels=F,coords=NULL,cex.coords=1, col.coords="red", labels=NULL, cex.labels=1, col.labels="blue",stack_eye=1.73,newcolhabs,width.legend=20,legbar.h=.2,z.value=NULL,zmin=0,zmax=11,legy.adj=0,leglabpos=.3,legtitle.pos=.2,col_layer="gray",col_base="white",xlim=NULL,ylim=NULL,xlab="Longitude", ylab="Latitude", zlab="Layer",ticktype="detailed",...) {
...

  {
    ...
    pmat<-persp(xx,yy,z, border=NA, zlim=c(zmin,zmax), xlab=xlab, ylab=ylab, zlab=zlab,ticktype=ticktype, r=stack_eye,xlim=xlim,ylim=ylim,...)
  }
  ...
  for(i in 1:length(rlist)){
   ...
   try({
     ...
     if(ncol(attrs)>1){
       lab<-attrs[,2]
       newcol<-adjustcolor(getcolhabs(newcolhabs,col1,length(attrs[,2])), transp)
       newcol<-newcol[which(as.numeric(as.factor(attrs[,2]))%in%    na.omit(rlist0[[i]]@data@values))]
       color= colorRampPalette(newcol)(100)
     }
   })
    nrz <- nrow(z)
    ncz <- ncol(z)
  ...
  try({
    for(i in z.value)
    {
      ...
      attrs<-rlist0[[pic]]@data@attributes[[1]]
      if(ncol(attrs)>1){
        lab<-attrs[,2]
      } else{
      ...
      #par(new=T)
     # points(seq(1,50),rep(i,50), pch=15,  col=co, cex=3)

      legend_image <- as.raster(matrix(getcolhabs(newcolhabs,col1,50), ncol=50))


      rasterImage(legend_image, 0.3, i-legbar.h-legy.adj, 50,i-legy.adj)
      text(x=seq(1,50,len=length(lab)),y=rep(i-leglabpos-legy.adj,length(lab)),lab, xpd=T)
      text(x=25,y=i-legy.adj+legtitle.pos,names(rlist)[i], xpd=T)
      #abline(h=i+.3, lty=2, col="gray")
      #title(main=attr(rlist[[i]],"get"), xpd=T)
      #axis(1)
      #legend("top",legend=attr(rlist[[i]],"get"),adj=c(0,-1), bty="n", xpd=T)
      # title(main=attr(rlist[[i]],"get"), xpd=T)

      # lab<-round(quantile(na.omit(rlist0[[i]]@data@values), c(0,.33,.66,1)))
      # mtext(lab,1,at=seq(1,50,len=4), line=1)
...
get_4D<-function(my_rst1,my_rst2=NULL,colors,exp=0.2,wlegend=20,hlegend=20,...)
{
...
par(mar=c(1,4,1,1))
  ...
  z[nas]<-NA
  pmat<-persp(x=x,y=y,z=z, col=cols, border=NA,exp=exp,ticktype='detailed',...)
