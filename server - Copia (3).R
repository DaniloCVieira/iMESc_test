...
tipify(span('Breakpoints'), "The break points computed", placement = "right")
  ...
  output$down_raster_btn<-renderUI({
    ...
    downloadButton('down_raster',tipify(icon("fas fa-download"),"Download plot"), style="button_active")
  })
  ...
  observeEvent(input$dropID_tool4,{
  ...
  observeEvent(input$dcogs8,{
    showModal(
      modalDialog(easyClose = T,
                  title=strong("Merge Columns"),
                  div(
                    tags$head(
                    ...
                    fluidRow(
                      column(
                        ...
                        column(
                          ...
                          tags$p("input$rank_list_cols_1"),
                          verbatimTextOutput("results_1"),
                          
                          tags$p("input$rank_list_cols_2"),
                          verbatimTextOutput("results_2"),
                          
                          tags$p("input$bucket_cols"),
                  ...
                  div(
                    div(class="cogs_in",
                      div(style="overflow-y: scroll;height: 150px;overflow-x: scroll; padding-top: 10px",
                        checkboxGroupInput('delete_picker',NULL,names(vals$saved_data),width='300px')
                      )
                    ),
                  bsButton("delete_datalist",icon=icon("far fa-trash-alt"),"Delete", style="button_active", block=T, value=F)
                  )
      )
    )
...
new
  ...
  observeEvent(input$prev_import, navpage_imp(-1))
  observeEvent(input$next_import, navpage_imp(1))
  ...
  hand_save_modal<-reactive({
    ...
    tags$div(id="savemodal",
             ...
             modalDialog(
               ...
               title=span(icon("fas fa-save"),'Save'),
  ...
  output$datalistname<-renderUI({
    inline(textInput("data_name", NULL, value=paste0(if(input$up_or_ex == 'use example data'){'nema_araca'} else {gsub(".csv","",input$filedata$name)}), width= '320px'))
  })
  ...
  output$insert_page2<-renderUI({
    div(
      div(
        datalist_render(getdatalist()[[1]],F)
      )
    )
  })
  observe({
  ...
  observeEvent(input$insert_cancel,{
    removeModal()
  })
  ...
  output$insert_page2<-renderUI({datalist_render(getdatalist()[[1]],F)})
  ...
  mybooks<-reactive({
    validate(need(length(grep(".rds",input$load_savepoint$datapath))==length(input$load_savepoint$datapath),"Requires rds file"))
    #a<-"D:/OneDrive/PÓS-DOC/Articles/MS_meiofauna/Submission/Dados/Results/Meio_raw_replicates.rds"
             ...
             inline(
               div(
                 conditionalPanel("input.model_or_data=='som codebook'",
                                  {
                                    inline(uiOutput("somHC"))
                                  }),
                 
                 conditionalPanel("input.model_or_data=='data'",
                                  iline(uiOutput("disthc"))
  ...
  output$choicesHC<-renderUI({
    ...
    radioButtons("model_or_data", "Clustering target", choiceValues  = choices_hc(), selected=c(choices_hc()[length(choices_hc())]), width="200px",choiceNames =choices_hc2())
  })
  ...
  observeEvent(input$fheight,{vals$cur_fheight<-input$fheight})
  observeEvent(input$fwidth,{vals$cur_fwidth<-input$fwidth})
  observeEvent(input$fres,{vals$cur_fres<-input$fres})
  observeEvent(input$pointsize,{vals$cur_pointsize<-input$pointsize})
  observeEvent(input$fformat,{vals$cur_fformat<-input$fformat})
    ...
    modalDialog(
      column(12,
             splitLayout(
               ...
               selectInput("fformat", "File type", choices=c("png","tiff","jpeg","pdf"), selected = vals$cur_fformat, multiple = FALSE, selectize = TRUE),
  ...
  output$get_data_upload<-renderUI({
    div(class="data_change0",
        ...
        div(id="data_change",
            ...
            inline(bsButton("reset_change",icon("fas fa-undo"), style="button_change"))
            ),
  ...
  observeEvent(input$reset_change,{
...
observeEvent(input$savechanges_return,{
  ...
  output$tools_upload<-renderUI({
    ...
    choiceNames=list(
      ...
      div(id="tools_drop3",class="needed",div(cogs_title="Transformation",
  ...
  output$som_type<-renderUI({
    if(is.null(vals$cur_somtype)){vals$cur_somtype<-'Unsupervised'}
    ...
    if(input$som_tab=="som_tab1"){
      div(
        inline(div(
          ...
          div(pickerInput("som_test_pick",NULL, choices=c("None", colnames(attr(vals$saved_data[[input$data_som]],"factors"))),selected=vals$cur_partsom,width="150px",))
        )),inline(uiOutput("som_test_ref"))
      )
    }
  })
           ...
           column(12,
                  fluidRow(
                    column(12,
                           splitLayout(cellWidths = c('10%','90%'),
                                       checkboxInput(
                                         "sugtopo", NULL, value =tunesom$sugtopo
                                       ),
